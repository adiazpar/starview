/**
 * MoonPhaseGraphic Component
 *
 * Renders an accurate SVG representation of the moon's current phase and rotation.
 * Uses a realistic moon texture as base with an accurate shadow overlay.
 *
 * Shadow calculation accounts for:
 * - Illumination percentage (determines shadow size)
 * - Waxing vs waning (determines which side the shadow is on)
 * - Rotation angle (orients the moon as seen from observer's location)
 */

import './styles.css';

/**
 * Calculate the SVG path for the moon's shadow based on illumination and phase direction.
 *
 * The shadow is bounded by:
 * 1. The terminator - an ellipse that varies from straight (quarter) to circular (new/full)
 * 2. The moon's edge - a semicircle on the shadow side
 *
 * @param {number} illumination - Percentage of moon illuminated (0-100)
 * @param {number} radius - Radius of the moon circle
 * @param {boolean} isWaning - True if moon is waning (shadow on right), false if waxing (shadow on left)
 * @returns {string} SVG path data for the shadow
 */
function calculateShadowPath(illumination, radius, isWaning) {
  const f = illumination / 100;

  // Full moon - no shadow
  if (f >= 0.99) return '';

  // New moon - entire disc is shadow
  if (f <= 0.01) {
    return `M 0 ${-radius} A ${radius} ${radius} 0 1 1 0 ${radius} A ${radius} ${radius} 0 1 1 0 ${-radius}`;
  }

  // The terminator is an ellipse with:
  // - Semi-major axis = radius (vertical, always full height)
  // - Semi-minor axis = |2f - 1| * radius (0 at quarter, approaches radius at new/full)
  const curvature = Math.abs(2 * f - 1);
  const terminatorRx = curvature * radius;

  // The terminator bows toward the shadow side when f > 0.5 (gibbous phases)
  // and toward the lit side when f < 0.5 (crescent phases)
  const bowTowardShadow = f > 0.5;

  if (isWaning) {
    // Shadow on RIGHT side
    // Path: top center → terminator to bottom → right semicircle back to top
    // Terminator: bows right (toward shadow) when f > 0.5, so sweep=1
    //             bows left (toward lit) when f < 0.5, so sweep=0
    // Moon edge: sweep=0 for right semicircle (counter-clockwise from bottom to top)
    const terminatorSweep = bowTowardShadow ? 1 : 0;
    ...
    // Path: top center → terminator to bottom → left semicircle back to top
    // Terminator: bows left (toward shadow) when f > 0.5, so sweep=0
    //             bows right (toward lit) when f < 0.5, so sweep=1
    // Moon edge: sweep=1 for left semicircle (clockwise from bottom to top)
    const terminatorSweep = bowTowardShadow ? 0 : 1;
...
export default function MoonPhaseGraphic({
  illumination = 50,
  isWaning = false,
  rotationAngle = 0,
  size = 180,
  ...
  // The base SVG has viewBox="0 0 128 128" with moon centered at (64, 64) radius 60
  // We'll use a centered coordinate system for easier shadow calculation
  const radius = 60;
  const shadowPath = calculateShadowPath(illumination, radius, isWaning);

  return (
    <svg
      className={`moon-graphic ${className}`}
      viewBox="0 0 128 128"
      width={size}
      height={size}
      style={{ transform: `rotate(${rotationAngle}deg)` }}
      aria-label={`Moon at ${illumination.toFixed(0)}% illumination`}
    >
      {/* Moon base - realistic texture from SVG file */}
      <g className="moon-graphic__base">
        {/* Main moon disc */}
        <circle cx="64" cy="64" r="60" fill="#fce5ac" />

        {/* Surface details / maria (dark patches) */}
        <path
      ...
      {/* Shadow overlay - translated to center of moon (64, 64) */}
      {shadowPath && (
